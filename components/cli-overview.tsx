...
const CLI_LINES = [
  ...
  { prompt: false, text: "-> Confirm this code in your browser: QKR4-6BUL" },
  { prompt: false, text: "OK Logged in." },
  { prompt: true, text: "envshare clone acme/api-gateway" },
  { prompt: false, text: "Cloning git@github.com:acme/api-gateway.git..." },
  { prompt: false, text: "OK Wrote api-gateway/.env" },
];

export function CliOverview() {
  const t = useTranslations("cliPage.overview");
    ...
    const ctx = gsap.context(() => {
      ...
      mm.add(
        ...
        (context) => {
          ...
          gsap.from("[data-cli-overview=text]", {
            y: 20,
            opacity: 0,
            duration: 0.6,
            ease: "power3.out",
          ...
          gsap.from("[data-cli-overview=terminal]", {
            y: 24,
            opacity: 0,
            duration: 0.6,
            ease: "power3.out",
          ...
          gsap.from("[data-cli-overview=line]", {
            opacity: 0,
            stagger: 0.12,
            duration: 0.3,
            ease: "power1.out",
  ...
  return (
    <div ref={rootRef} className="bg-background py-16 lg:py-24">
      <section
        aria-labelledby="cli-overview-heading"
        className="mx-auto grid max-w-7xl grid-cols-1 items-center gap-10 px-4 sm:px-6 lg:grid-cols-2 lg:gap-16 lg:px-8"
      >
        <div data-cli-overview="text" className="flex flex-col gap-5">
          <h2
            id="cli-overview-heading"
